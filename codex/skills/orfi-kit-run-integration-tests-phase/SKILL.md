...
- Windows (PowerShell): `powershell.exe -Command "Start-Process pwsh -ArgumentList '-NoExit', '-Command', \"Get-Content '<ABSOLUTE_PATH_TO_REPO>/.tests/integ-tests-log.log' -Wait -Tail 50\""` — resolve the absolute path from the current directory before embedding.
