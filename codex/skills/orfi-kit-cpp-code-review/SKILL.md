...
description: "Run a C++ code review that executes the enforcing tools (clang-format / clang-tidy / build / tests) and grounds style verdicts in the repo's own config rather than general C++ norms. Run orfi-kit-cpp-code-review [DIFF|FULL|RAW|SMART|BUGS|SECURITY|PERFORMANCE|<path>]."
---
...
review — no `clang-format -i`, no reconfiguring the build.
...
3. **`CODEX-SESSION-STATE.md`** — the handoff file
...
- **A security pass** — Codex has no bundled security-review skill, so note security as
...
**On Codex it is deliberately NOT wired as a Stop hook.** Codex has a Stop event and a native
hooks file (`~/.codex/hooks.json`), but this contract is left unwired there: Codex's transcript
format is not a stable interface for hooks, and the verifier's honesty guard would refuse to run
against a transcript it does not recognize — a hook that always passes is strictly worse than no
hook. OpenCode has no Stop event, so it is not enforced there either. When the native hooks are not
installed the discipline is yours:
...
**Large diffs — split the work.** When the diff is too big for one pass, break it up deliberately: by
file, or by dimension (correctness, memory/lifetime, headers, completeness). Carry the config you read
...
3. **The kit baseline in `CONFIG.md`** — when the repo encodes nothing, this is the contract, not a
   suggestion. The guardrails in `~/.codex/AGENTS.md` already instruct the author to treat it as the
   contract and report deviation as a violation, so reviewing the same code as though no rule
   existed would contradict the standard the author was held to. Cite the baseline key. It never
   overrides repo config — it fills the vacuum, and only the vacuum.
