...
Call `atlassian-jira_get_issue` with the issue key the user gave you. Read the summary, description, acceptance criteria, issue type, labels, components, and current story-point value (if any).
...
**Is it a bug?** If `issuetype.name == "Bug"`, stop. Bugs don't get story points on this team — bug effort is unpredictable and lumping it with feature velocity distorts sprint planning. Explain this to the user, then add a comment to the ticket via `atlassian-jira_add_comment` noting that the ticket was reviewed for estimation and skipped per the bugs-no-points policy. Do not call `atlassian-jira_update_issue`.
...
**Never call `atlassian-jira_update_issue` without explicit confirmation from the user.** Writing story points is reversible in Jira but noisy — it triggers notifications and shows up in sprint reports. Ask, then act.
...
```
atlassian-jira_search_fields  query: "story points"
```
...
atlassian-jira_update_issue
  issueKey: <KEY>
  customFields: { "<customfield_id>": <number> }
```
