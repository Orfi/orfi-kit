...
> Sync a `feature/*` branch through its full parent hierarchy (`origin/master` -> `epic/*` -> `feature/*`) before you push.
...
Run it manually from a session whose current worktree is on a `feature/*` branch:
...
- The current worktree must be on a `feature/*` branch. If it is not, the command aborts immediately.
- A remote named `origin` with `git fetch` available.
- Branch naming convention: `feature/{id}-{name}` and `epic/{id}-{name}`.
...
1. **Verify branch** — confirms the current branch matches `feature/*`; aborts otherwise. Records `FEATURE_BRANCH` and `FEATURE_WORKTREE`.
...
This command is one half of a pair. The companion `orfi-kit-enforce-sync.sh` is a `PreToolUse` hook on `Bash` calls that **blocks** any `git push` when the current branch is `feature/*` and its parent epic has moved ahead. The hook reads the same `.claude/hooks/state/parent-epic` file this command writes (or discovers the epic dynamically), then runs `git merge-base --is-ancestor origin/<epic> HEAD`; if the epic tip is not an ancestor of the feature HEAD, the push is denied with a message telling you to run `/orfi-kit-sync-branch`. Running this command is what clears that block.
