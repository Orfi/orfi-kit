...
- **Opening the story PR (feature → epic):** sync the cascade, run the project's own quality gate (command per the project's onboarding docs), open PR with base = epic branch (never master). After merge, prune: `git worktree remove`, `git branch -D`, `git push origin --delete` the feature branch.
