...
- **C++-specific judgment** — memory and lifetime (ownership clarity, raw `new`/`delete` where a smart pointer or Qt parent-child ownership belongs, missing virtual destructors, rule of three/five/zero, leaks on early-return and exception paths), const correctness, and header hygiene (self-containment, include order, includes that should be forward declarations, guards present and matching).
