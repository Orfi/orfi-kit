...
- The seven hook scripts deployed to `~/.claude/hooks/` (the installer does this when any runtime that needs them is selected — Copilot included). On Windows the `powershell` field invokes Git Bash explicitly and must be present; both fields carry the script as an **absolute forward-slash path** baked at install time.
...
- `env` carries `ORFI_HOOK_PLATFORM=copilot`. Both `bash` and `powershell` fields target the script with an **absolute forward-slash path baked in by the installer** — the shipped `orfi-kit.json` is a template whose `@ORFI_COPILOT_HOME@` placeholder is replaced at install time (`cygpath -m "$HOME"` in the sh installer, `$env:USERPROFILE` in the ps1 installer). The `powershell` field wraps it **inside a single-quoted `bash -c` body** (`& bash -c 'bash "C:/Users/…/.claude/hooks/…"'`), so a Windows-style path never crosses the PowerShell→Git Bash argument boundary. Two hard-won rules keep these hooks green on Windows:
  ...
  - **Do not pass the path as a separate argument.** `& bash "$env:USERPROFILE\.claude\hooks\…"` and even `& bash "C:/Users/…."` get mangled/stripped at the PowerShell→Git Bash boundary (backslashes stripped → `C:Users…: No such file`, args dropped entirely), and every such hook then fails **closed** in Copilot — one mangled hook denies all tool calls. Keep the single-quoted `bash -c '…'` form with the path embedded in the body exactly as shipped.
