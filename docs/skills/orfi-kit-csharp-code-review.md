...
- Optional: a source of truth for intent (see below). Nothing here is required — the review degrades gracefully and asks you when it finds nothing.

## Behavior / rules

- **Scope** — defaults to the diff: **the whole body of work on the branch**, which in practice is a story or a phase — everything that will land when the branch merges. The base is the branch's fork point from its parent: its `epic/*` branch when it derives from one, otherwise the trunk. It deliberately does **not** use `git merge-base HEAD @{u}`, which resolves to the last *pushed* commit and would silently omit already-pushed work — reviewing a fraction of the story while appearing to review all of it. Reviewing the whole-solution (`FULL`) is the unlikely case, for onboarding to unfamiliar code or a deliberate sweep rather than ordinary pre-PR review. It asks when the default is unusable (empty diff, no resolvable base, or a diff big enough that whole-project tooling is cheaper) and always reports the scope and the base it resolved to.
...
- **Finds the source of truth before judging intent** — correctness and completeness need something to compare the code against; without it a reviewer infers intent from the diff and then judges the diff against that inference, so everything looks complete. It works down a ladder: `ONBOARDING.md` (this kit's own project reference, created by `orfi-kit-init` — it names the ADR/spec location, test commands, and security gate, so it usually points at the rest) → the session-state handoff file written by `orfi-kit-persist-state`, which often records what a prior session was building and from which plan → PRD or spec → ADRs → a plan, in whatever shape the repo happens to use → PR body, commits, or linked ticket → tests as executable spec → public API contracts. **If it can't find any of them it asks you**, since you usually know where intent lives even when the repo doesn't advertise it.
- **Kit-agnostic by construction** — it learns where plans live from *this* kit's own files (`ONBOARDING.md`, the session-state handoff) or from you, never by knowing another tool's directory layout. A plan file already in the repo is just an input to read, whoever wrote it; no particular format is required and no other kit is ever invoked to obtain one.
- **Completely resilient** — every rung is optional. Missing, empty, stale, or mutually contradictory sources are all normal: it prefers the higher rung, notes the contradiction, and continues. It never blocks, errors, or refuses to review because a document wasn't there, and never treats a document's absence as a defect in the code under review.
- **Degrades rather than refuses** — with no source of truth it says "completeness unverifiable", names the strongest rung it did find, and reviews everything that doesn't need a spec: the whole tool lane, internal correctness, and test coverage. A missing spec narrows the review without stopping it — but a narrowed review is never reported as a clean bill of health, and it won't invent the intent it couldn't find.
- **Correctness splits in two** — *internal* correctness needs no spec and always runs (null and error paths, boundaries and off-by-ones, concurrency, cancellation, disposal, swallowed exceptions, unreachable branches). *Intent* correctness — does it do the right thing, not just a consistent thing — requires a rung from the ladder, and the review names which one it judged against.
...
Intent: ONBOARDING.md → docs/adr/0007-retry-policy.md (rung 4 of 8)
