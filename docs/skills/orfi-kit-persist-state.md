...
- Writes your current state to `<helper-files-root>\CLAUDE-SESSION-STATE.md`.
...
- Pairs with `/orfi-kit-load-state`, which reads the same `<helper-files-root>\CLAUDE-SESSION-STATE.md` file in full at the start of a new session to restore the handoff.
