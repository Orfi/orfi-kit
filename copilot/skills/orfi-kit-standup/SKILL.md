...
description: Build a status/standup report through guided questions, then format it for Teams. Use this skill whenever the user asks to write, build, or format a standup, status report, or progress update — phrases like "standup", "status report", "what I've been working on", "write my update for Teams", or any request to turn work-in-progress into a shareable status. Fetches Jira titles via the Atlassian MCP server when the user names a ticket.
---
