...
description: Foundational behavioral constraints for all Copilot interactions in this project. Enforces honesty, transparency, safe version control practices, and clear communication. This skill should always be active — it governs how Copilot operates regardless of the specific task. Apply these constraints whenever working on code, resolving conflicts, communicating progress, or making decisions that affect the codebase.
---
...
Exit codes lie. A test runner can return exit code 0 while the actual output shows failures, skipped tests, or partial runs. This has caused repeated incidents where Copilot reported "tests passed" based on the exit code alone, only for the user to discover failures were hiding in the output. That pattern destroys trust fast.
