---
name: orfi-kit-run-unit-tests-phase
description: Run unit tests for a GSD phase, or all phases if no number given.
---
...
Log file path: `.tests/unit-tests-log.log` (relative to repo root).

Create the `.tests/` directory if it doesn't exist (`mkdir -p .tests`, or `New-Item -ItemType Directory -Force .tests` in PowerShell).

## Determine scope

If PHASE_NUMBER is provided, run unit tests for that GSD phase only.
If PHASE_NUMBER is not provided, run unit tests for ALL GSD phases.
...
- Windows (PowerShell): `powershell.exe -Command "Start-Process pwsh -ArgumentList '-NoExit', '-Command', \"Get-Content '<ABSOLUTE_PATH_TO_REPO>/.tests/unit-tests-log.log' -Wait -Tail 50\""` — resolve the absolute path from the current directory before embedding.
