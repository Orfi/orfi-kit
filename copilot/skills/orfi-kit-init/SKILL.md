...
If `<kit-root>` exists and already contains `COPILOT-SESSION-STATE.md` and `ONBOARDING.md`, then this repo is already initialized: tell the user so and **stop** — do not touch anything. (This is the Copilot skill, so it owns `COPILOT-SESSION-STATE.md`; the Claude command of the same name owns `CLAUDE-SESSION-STATE.md`.)
...
- `COPILOT-SESSION-STATE.md` — placeholder with guiding comment headers for what the session should record. Use this skeleton:

```
# COPILOT-SESSION-STATE.md — shared session handoff
