...
- Windows: `powershell.exe -Command "Start-Process pwsh -ArgumentList '-NoExit', '-Command', \"Get-Content '<ABSOLUTE_PATH_TO_REPO>/.tests/unit-tests-log.log' -Wait -Tail 50\""` — resolve the absolute path from `pwd` before embedding.