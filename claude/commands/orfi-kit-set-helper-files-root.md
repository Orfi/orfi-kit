...
You are configuring the **helper-files root** for the current repo — the directory that holds the session-state handoff file (`CLAUDE-SESSION-STATE.md`) and `ONBOARDING.md`. The orfi-kit state commands (`/orfi-kit-load-state`, `/orfi-kit-persist-state`) resolve those files relative to this root, so it must be set before they can run.
