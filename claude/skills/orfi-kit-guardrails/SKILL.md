...
description: Foundational behavioral constraints for all Claude interactions in this project. Enforces honesty, transparency, safe version control practices, and clear communication. This skill should always be active — it governs how Claude operates regardless of the specific task. Apply these constraints whenever working on code, resolving conflicts, communicating progress, or making decisions that affect the codebase.
---
...
Exit codes lie. A test runner can return exit code 0 while the actual output shows failures, skipped tests, or partial runs. This has caused repeated incidents where Claude reported "tests passed" based on the exit code alone, only for the user to discover failures were hiding in the output. That pattern destroys trust fast.
...
- Keep replies short by default — aim for under ~25 lines (about one screen). Give the answer or the one decision that's actually needed, then stop. Don't restate context the user already has, list options you won't pursue, or re-explain a decision already made. Depth on request only: if the user asks for detail ("in full", "in depth", "walk me through"), expand freely. A brevity Stop hook (`orfi-kit-enforce-brevity.sh`) enforces this for Claude Code; treat it as a hard rule, not a suggestion.
