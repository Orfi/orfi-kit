...
surfaces already hold authors to exactly that: `orfi-kit-load-csharp-conventions.sh` prints "ENFORCING
the orfi-kit baseline as the contract for this repo", and the Copilot guardrails extension instructs
the same. A rule enforced when code is written but ignored when it's reviewed is worse than no rule.
