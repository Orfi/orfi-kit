...
3. **The kit baseline in `CONFIG.md`** — when the repo encodes nothing, this is the contract, not a
   suggestion. The write-time conventions hook (`orfi-kit-load-csharp-conventions.sh`) already
   enforces it as the contract while the file is being written, so reviewing the same code as though
   no rule existed would contradict the gate the author was held to. Cite the baseline key. It never
