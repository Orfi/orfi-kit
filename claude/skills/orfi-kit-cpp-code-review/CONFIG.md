...
key. The write-time surfaces already hold authors to exactly that — `orfi-kit-load-cpp-conventions.sh`
prints "ENFORCING the orfi-kit baseline as the contract for this repo", and the Copilot guardrails
extension instructs the same. Reviewing that code as though no rule existed would contradict the gate
its author was held to, and a rule enforced at write time but ignored at review time is worse than no
rule at all.
