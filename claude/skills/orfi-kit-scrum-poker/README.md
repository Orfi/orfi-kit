...
A Claude Code skill that estimates Jira tickets using planning-poker story points via the Atlassian MCP server. When you ask Claude to estimate a ORFI ticket, the skill reads the ticket, pulls recent estimated tickets for calibration, proposes a number with reasoning, waits for your confirmation, and writes the value back to Jira.

## Install

1. Copy the `orfi-kit-scrum-poker/` folder into your Claude Code skills directory:
   - **Mac/Linux:** `~/.claude/skills/`
   - **Windows:** `C:\Users\<you>\.claude\skills\`
2. Restart your Claude Code session. The skill auto-registers.

## Prerequisite

You need the Atlassian MCP server configured with access to the Orfi Jira instance. The skill calls:
...
On first use, the skill auto-discovers your `Story Points` custom field ID via `jira_search_fields` — no manual config needed.

## How to use

Just ask Claude, naturally. Any of these will trigger the skill:
...
- Adds a comment to the ticket via `jira_add_comment` noting it was reviewed for estimation and skipped per the bugs-no-points policy
...
- Records the spike classification as a `jira_add_comment` so the context lives on the ticket
